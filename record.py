...
# Config defaults (note screen_speed replaces fps)
cfg = {
    "profile": "web",
    "format": ".mp4",
    "screen_speed": "Medium (20 fps)",
    "record_mic": True,
    "mic_device": None,
    "record_system": False,
    "system_device": None,
...
def apply_cfg_to_ui():
    try:
        ...
        screen_speed_var.set(cfg.get("screen_speed", "Medium (20 fps)"))
...
def record_worker(video_path, audio_path, final_path, cfg_local, stop_flag):
    ...
    # Map user-friendly screen_speed to capture FPS
    speed_map = {
        "Slow (10 fps)": 10,
        "Medium (20 fps)": 20,
        "Fast (30 fps)": 30,
...
def create_ui():
    ...
    # --- Screen Speed selector (replaces FPS) ---
    ttk.Label(gp, text="Screen Speed:").grid(row=0, column=2, padx=(16,4))
